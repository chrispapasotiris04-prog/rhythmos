...
function goodText(key,path,mime){return fetch(path+'?v='+Date.now(),{cache:'no-store'}).then(function(fr){if(!fr.ok)throw 0;return fr.text();}).then(function(t){var nr=new Response(t,{headers:{'Content-Type':mime}});caches.open(GOOD).then(function(c){c.put(key,nr.clone());});return nr;}).catch(function(){return caches.open(GOOD).then(function(c){return c.match(key);}).then(function(r){if(r)return r;return caches.open(LASTGOOD).then(function(lc){return lc.match('index.html');});});});}
...
self.addEventListener('activate',function(e){e.waitUntil(caches.keys().then(function(ks){return Promise.all(ks.filter(function(k){return [CACHE,FLAG,PENDING,LASTGOOD,GOOD].indexOf(k)===-1;}).map(function(k){return caches.delete(k);}));}).then(function(){return self.clients.claim();}));});
...
if(isManifest){e.respondWith(readForce().then(function(f){if(f==='1')return goodText('manifest','good/manifest.webmanifest.txt','application/manifest+json');return fetch(e.request);}));return;}
if(isIndex){e.respondWith(readForce().then(function(f){
if(f==='1'){return goodText('index','good/index.html.txt','text/html; charset=utf-8');}
...
e.respondWith(fetch(e.request).then(function(res){if(res.ok||res.type==='opaque'){var cp=res.clone();caches.open(CACHE).then(function(c){c.put(e.request,cp);});}return res;}).catch(function(){return caches.match(e.request).then(function(r){return r||Response.error();});}));});
